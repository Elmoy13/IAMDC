...
async def list_channels(
    agency_id: UUID,
    brand_id: UUID = Query(..., description="Brand to filter channels by"),
    user: dict = Depends(get_current_user),
    agency: dict = Depends(get_user_agency),
    db: AsyncSession = Depends(get_db),
):
    """List connected channels for a brand within an agency."""
    if str(agency["agency_id"]) != str(agency_id):
        raise HTTPException(403, "No access to this agency")
    return await channel_service.list_channels_by_agency(db, agency_id, brand_id)
